...
// and dynamically overridden by Canvas environment variables if available.
...
// Use Canvas provided config if available, else local config
const firebaseConfig = typeof __firebase_config !== 'undefined' && __firebase_config
    ? JSON.parse(__firebase_config)
    : localFirebaseConfig;

// Use Canvas provided app ID if available, else local project ID
const appId = typeof __app_id !== 'undefined' && __app_id
    ? __app_id
    : localFirebaseConfig.projectId; // Fallback to projectId as appId if __app_id is missing
...
const time12to24 = (time12h: string) => {
...
const formatTime24to12 = (time24h: string) => {
...
export default function App() {
    ...
    useEffect(() => {
        ...
        const unsubOneTime = onSnapshot(oneTimeQuery, (oneTimeSnapshot) => {
            const oneTimeBookings = oneTimeSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Booking));
            ...
            const unsubRecurring = onSnapshot(recurringQuery, (recurringSnapshot) => {
                ...
                const allBookings = [...oneTimeBookings, ...recurringBookings];
                    ...
                    <div className="grid grid-cols-1 divide-y divide-slate-100">
                        {loading ? (
                            ...
                            timeSlots.map(time => {
                                ...
                                // Check if this is the first slot of a multi-slot booking
...
function BookingModal({ onClose, selectedDate, selectedTime, dailyBookings, setMessage }: {
    ...
    // Removed local error state, will use setMessage for errors
    ...
    const checkOverlap = useCallback((newBookingTime: Date, newBookingDuration: number) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const time24 = time12to24(selectedTime); // Convert 12-hour selectedTime to 24-hour for storage
